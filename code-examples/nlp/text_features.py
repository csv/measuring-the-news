...
def text_features(text):
    ...
    return {
         ...
         "noun_per": pos_percentages(words,'N.+')  if words is not None and len(words)>10 else None,
         "verb_per": pos_percentages(words,'V.+')  if words is not None and len(words)>10 else None,
         "ger_per": pos_percentages(words,'J.+') if words is not None and len(words)>10 else None,
         "avg_word_syllables": avg_word_syllables(words) if words is not None and len(words)>10 else None,
         "avg_word_length": avg_word_length(words_lower) if words_lower is not None and len(words_lower)>10 else None,
         ...
         "liwc_future": liwc_count(words_lower, "future") if words_lower is not None and len(words_lower)>10 else None,
         "liwc_achieve": liwc_count(words_lower, "achieve") if words_lower is not None and len(words_lower)>10 else None,
         "liwc_pos": liwc_count(words_lower, "posemo") if words_lower is not None and len(words_lower)>10 else None,
         "liwc_neg": liwc_count(words_lower, "negemo") if words_lower is not None and len(words_lower)>10 else None,
         "liwc_money": liwc_count(words_lower, "money") if words_lower is not None and len(words_lower)>10 else None,
         "liwc_sexual": liwc_count(words_lower, "sexual") if words_lower is not None and len(words_lower)>10 else None,
         "liwc_quant": liwc_count(words_lower, "negemo") if words_lower is not None and len(words_lower)>10 else None,
         "liwc_health": liwc_count(words_lower, "health") if words_lower is not None and len(words_lower)>10 else None,
         "sentence_count": sentence_count(sents)  if sents is not None and len(sents)>2 else None,
         "avg_sentence_length": avg_sentence_length(sents) if sents is not None and len(sents)>2 else None,
         "ques_count": punct_count(text, '?') if text is not None and len(text)>100 else None,
         "excl_count": punct_count(text, '!') if text is not None and len(text)>100 else None,
         "quote_count": punct_count(text, '"') if text is not None and len(text)>100 else None,
         "flesh_readability": flesch_readability(text) if text is not None and len(text)>100 else None,
         "smog_readability": smog_readability(text) if text is not None and len(text)>100 else None,
         "coleman_liau_readability": coleman_liau_readability(text) if text is not None and len(text)>100 else None,
         "bias_word_count": bias_word_count(text) if text is not None and len(text)>100 else None,
         "number_of_grafs": number_of_grafs(grafs) if grafs is not None and len(grafs)>0 else None,
         "length_of_first_graf": length_of_first_graf(grafs) if grafs is not None and len(grafs)>0 else None,
         "avg_graf_length": avg_graf_length(grafs) if grafs is not None and len(grafs)>0 else None
    }

if __name__ == '__main__':
  articles = [i['content'] for i in json.load(open(sys.argv[1]))]
  for article in articles:
    print text_features(article)
